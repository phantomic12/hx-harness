...
1501 tests, 0 failed                     # includes 25 chat API tests and 4 database reopen tests
...
```bash
cargo test --workspace --locked   # 1501 tests, 0 failed, 60 ignored live tests
...
cargo test -p hx-server         # 155 — routes, the loop end to end over HTTP, the M8 child spawner, and the diff/review pane
