...
1438 tests, 0 failed                     # includes 25 chat API tests and 4 database reopen tests
...
```bash
cargo test --workspace --locked   # 1438 tests, 0 failed, 60 ignored live tests
...
cargo test -p hx-core           # 154 — classification, the policy ladder, and the ceiling comparison
...
- **13 crates with logic**: unit-tested at the level of pure functions and in-process lifecycles. There
  ...
  (A 14th, `hx-desktop`, carries 11 tests for its endpoint/token resolution and bundle reuse.)
