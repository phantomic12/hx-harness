...
1492 tests, 0 failed                     # includes 25 chat API tests and 4 database reopen tests
...
```bash
cargo test --workspace --locked   # 1492 tests, 0 failed, 60 ignored live tests
...
cargo test -p hx-server         # 150 — routes, the loop end to end over HTTP, and the M8 child spawner + re-route (run_lane)
