...
| `hx-search` | 79 | 3114 | RRF rank fusion, HTML extraction, entity decoding, per-backend failure isolation (with **fake** backends). Plus **three more keyless backends**, each parsed from a captured response rather than from a shape the parser and the test agree on: Wikipedia against a **real live JSON capture** (the `searchmatch` spans stripped, `formatversion=1` pinned, a `site:` filter applied to the returned URLs because MediaWiki has no such operator), Marginalia against a **real HTML capture** (25 title anchors / 25 snippets, `<wbr>` removed as the word-break *opportunity* it is, no `&count=` because it was tried live and empties the result set), and Mojeek against a **transcription of its markup** — its live path is bot-walled from this environment (`curl` UA → 403, browser UA → 200 with `<title>Captcha</title>`) and has **not been exercised**, which the module doc and this row both say. Recency is deliberately not forwarded by Mojeek (its advanced form exposes no date-range parameter, only a `date` *display* toggle) and a test pins that. The Wikipedia article URL escapes `&` as `%26` while leaving `+` alone, pinned on the final URL string — an earlier comment justified the escape with a false claim (a bare `&` does not start a query string; `Url::parse("…/AT&T").query()` is `None`), so the reasoning in the code is now the measured one |
