...
1498 tests, 0 failed                     # includes 25 chat API tests and 4 database reopen tests
...
stays down for later draws. It is **not** a fan-out: neither method runs an agent loop, dispatches tools, or
...
```bash
cargo test --workspace --locked   # 1498 tests, 0 failed, 60 ignored live tests
