...
1589 tests, 0 failed                     # includes 25 chat API tests and 4 database reopen tests
62 ignored                               # live: Docker, SSH, pty, WinRM, search, a real model, a real bot, Firecracker
...
#
# The two totals above were re-measured on the `feat/m4-egress-host-hardening` tree by running
# the whole suite and summing per-suite lines (they read 1561/61 on `feat/docs-truth-sweep`,
# c52bc1b; the rest of the drift is intervening lanes, and 5 of it is the egress tripwire below).
...
**Host egress hardening: the provisioning step that closes the bridge-address hole**
(`scripts/harden-sandbox-egress.sh`, tripwire `crates/hx-sandbox/tests/egress_hardening_rules.rs`)

The runtime deliberately does not close the hole above, so this script is the supported closure:
`--apply SUBNET PROXY_PORT` as root on each sandbox host (idempotent), `--check` to re-verify,
`--revert` to remove exactly what `--apply` added, `--print-rules` to print the lines without
touching anything. The same policy lands in two chains because the destinations take different
netfilter paths: `DOCKER-USER` for the forwarded path and `INPUT` for traffic to addresses the
host itself owns — the bridge address is delivered locally and never traverses `DOCKER-USER`, so
forward-path rules alone would leave the measured sshd hole open while printing green checks.
Order in each chain is ACCEPT established/related, ACCEPT the gateway on the proxy port (the
sidecar path survives), DROP the gateway on everything else, DROP the other private ranges.

What was actually run (no root, no daemon, no network — the strongest check available here):
`sh -n` clean; `--print-rules 10.200.7.0/24 3128` byte-identical to the worked example in the
script header; gateway derivation checked for `/16` and `/26` as well as `/24`; eight malformed
inputs (bad port, bare IP, `/33`, `/31`, octet `256`, IPv6, non-numeric port) all refused with
exit 2 and a naming message; `--check`/`--apply`/`--revert` refused without root with exit 4;
and a full apply → check(exit 0) → re-apply (all `present:`, no duplicates) → revert → check
(exit 1 listing the missing lines, zero rules left) cycle against a stub `iptables`/`id` on
`PATH` in `/tmp`, with the post-apply chain head reading in firewall order. Not run: against a
real host firewall with a live sandbox probing `nc -vz <bridge> 22` — that per-host run is the
operator's step, named in the script header and in `ROADMAP.md`.

The tripwire (`crates/hx-sandbox/tests/egress_hardening_rules.rs`, in CI on all three OSes)
`include_str!`s the script and pins the exact rule fragments for the sample subnet — the gateway
DROP, the proxy-port ACCEPT, both chain anchors. On unix it additionally executes `--print-rules`
and compares the twelve lines byte for byte (pure computation, no root); on Windows, where there
is no `sh`, the twin pins the same lines through the header's worked example. Proven able to
fail: softening the gateway DROP to ACCEPT in the script reddened exactly the two rule-pinning
tests, and reverting restored green. `remote_live.rs`'s hole-asserting test is untouched — on a
host where this script has been applied it fails, which is the good-news signal, not a regression.
...
```bash
cargo test --workspace --locked   # 1584 tests, 0 failed, 60 ignored live tests
...
cargo test -p hx-server         # 155 — routes, the loop end to end over HTTP, the M8 child spawner, re-route, fan-out, and the diff/review pane
