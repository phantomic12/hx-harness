...
## M7 — Native apps
  ...
  - ✅ **Desktop: system tray, global hotkey, OS notifications, native file picker** — landed. The tray menu is a pure
    ...
    session and redacts token-shaped values and paths outside the workspace; and the **native file picker**
    keeps its decision logic (cancel vs not-a-directory vs accept, plus the unavailable-dialog
    degradation) in a pure, headlessly-tested core with the OS dialog as a thin shell. Each degrades
    to a working window with a warning rather than failing to start. The tray icon, a live hotkey
    binding, a raised notification and the live OS dialog are not exercised headlessly — each module's doc
    says so.
