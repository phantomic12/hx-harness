...
def main():
    ...
    # 6. An unknown terminal is a 404, which is what the page distinguishes from a dead shell.
    status, _ = http("GET", "/v1/terminals/definitely-missing/ws")
    check("an unknown terminal is refused", status == 404, str(status))

    # 7. The session socket the right pane uses, and its resumable handshake.
    status, body = http("POST", "/v1/sessions")
    session = None
    if status in (200, 201):
        session = json.loads(body).get("id") or json.loads(body).get("session")
    if not session:
        status2, body2 = http("GET", "/v1/sessions")
        items = json.loads(body2)
        items = items.get("sessions", items) if isinstance(items, dict) else items
        if items:
            session = items[0].get("id") if isinstance(items[0], dict) else items[0]
    check("a session exists to watch", bool(session), f"{status} {body[:120]}")
