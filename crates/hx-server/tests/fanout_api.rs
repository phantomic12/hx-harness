...
fn member(id: &str) -> PoolMember {
    PoolMember {
        id: id.to_string(),
        // These tests pin fan-out shape over HTTP, not member routing: the provider is the id
        // so the scripted registries (keyed by id) resolve unchanged, and the model is the id
        // so the route responses keep their names. Routing with distinct names is pinned in
