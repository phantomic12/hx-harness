...
pub async fn run_chat_with_session_notify(
    ...
    let approver: Arc<dyn Approver> = match level {
        ...
        // A run that asked how long it will wait for a human gets a queue whose silence horizon
        // matches the request. The question still lands on the daemon's shared queue (so a client polling
